...
//!
//!   S12FTMRC32K1V1  -  32K P-flash ( 64x512B sectors), 4K D-flash (16x256B sectors)
//!   S12FTMRC48K1V1  -  48K P-flash ( 96x512B sectors), 4K D-flash (16x256B sectors)
//!   S12FTMRC64K1V1  -  64K P-flash (128x512B sectors), 4K D-flash (16x256B sectors)
//!   S12FTMRC128K1V1 - 128K P-flash (256x512B sectors), 4K D-flash (16x256B sectors)
//!
...
void asm_entry(void);
const FlashProgramHeader_t xxflashProgramHeader;

// Used by programmer to locate flashProgramHeader 
const FlashProgramHeader_t *const headerPtr = &xxflashProgramHeader;
...
void setErrorCode(uint8_t errorCode) {
   // ENTRY : B = errorCode
   asm {
      ldx   xxflashProgramHeader:FLASH_DATA_OFFSET,PCR   // flashData
...
void makeLinearAddress(uint32_t *address) {
   ...
   tryDFlash:
      ...
      // A=PPage #, B=Address[15:8]
      cmpb  #0x04                  // Check if 0x0400-0x13FF
      blo   error                  // No  - error
      cmpb  #0x14
      bhi   error                  // No  - error
...
void programRange(void) {
   ...
   FlashData_t     *flashData;  
   asm {
      ldd   xxflashProgramHeader:FLASH_DATA_OFFSET,PCR   // flashData
      std   flashData
   }
...
void verifyRange(void) {
   ...
   // Verify words
   while (numWords-- > 0) {
      if (readLinearWord(address) != *data++)
...
void blankCheckRange(void) {
   ...
   while (numWords>0) {
      if (readLinearWord(address) != 0xFFFF)
...
void testApp(void) {
   FlashProgramHeader_t *fph = (FlashProgramHeader_t*) &xxflashProgramHeader;
   
   // Disable COP
   *(xxflashProgramHeader.copctlAddress) = 0;
